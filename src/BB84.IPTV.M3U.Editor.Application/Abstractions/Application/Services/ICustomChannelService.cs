...
{
	/// <summary>
	/// Gets all stored custom channels, ordered by name.
	/// </summary>
	/// <param name="cancellationToken">The cancellation token, for cancelling the operation if needed.</param>
	/// <returns>The stored custom channels.</returns>
	Task<IReadOnlyList<CustomChannelResponse>> GetChannelsAsync(CancellationToken cancellationToken = default);
