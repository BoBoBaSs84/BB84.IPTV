...
{
	[TestMethod]
	public async Task GetRegionsAsyncShouldLogAndRaiseEventOnException()
	{
		CancellationToken cancellationToken = CancellationToken.None;
		_httpClientFactoryMock.Setup(x => x.CreateClient(Constants.HttpClientName))
			.Throws<Exception>();

		IEnumerable<RegionRequest> result = await _sut
	...
	[TestMethod]
	public async Task GetRegionsAsyncShouldLogAndNotifyOnNotSuccessful()
	{
		CancellationToken cancellationToken = CancellationToken.None;
		_httpClientFactoryMock.Setup(x => x.CreateClient(Constants.HttpClientName))
