...
{
	[TestMethod]
	public async Task GetCitiesAsyncShouldLogAndRaiseEventOnException()
	{
		CancellationToken cancellationToken = CancellationToken.None;
		_httpClientFactoryMock.Setup(x => x.CreateClient(Constants.HttpClientName))
			.Throws<Exception>();

		IEnumerable<CityRequest> result = await _sut
	...
	[TestMethod]
	public async Task GetCitiesAsyncShouldLogAndNotifyOnNotSuccessful()
	{
		CancellationToken cancellationToken = CancellationToken.None;
		_httpClientFactoryMock.Setup(x => x.CreateClient(Constants.HttpClientName))
	...
	[TestMethod]
	public async Task GetCitiesAsyncShouldReturnDataWhenSuccessful()
	{
		CancellationToken cancellationToken = CancellationToken.None;
		_httpClientFactoryMock.Setup(x => x.CreateClient(Constants.HttpClientName))
			.Returns(CreateMockedClient(HttpStatusCode.OK, Resources.CityJson));
