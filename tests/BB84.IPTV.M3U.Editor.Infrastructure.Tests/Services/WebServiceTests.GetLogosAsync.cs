...
{
	[TestMethod]
	public async Task GetLogosAsyncShouldLogAndRaiseEventOnException()
	{
		CancellationToken cancellationToken = CancellationToken.None;
		_httpClientFactoryMock.Setup(x => x.CreateClient(Constants.HttpClientName))
			.Throws<Exception>();

		IEnumerable<LogoRequest> result = await _sut
	...
	[TestMethod]
	public async Task GetLogosAsyncShouldLogAndNotifyOnNotSuccessful()
	{
		CancellationToken cancellationToken = CancellationToken.None;
		_httpClientFactoryMock.Setup(x => x.CreateClient(Constants.HttpClientName))
	...
	[TestMethod]
	public async Task GetLogosAsyncShouldReturnDataWhenSuccessful()
	{
		CancellationToken cancellationToken = CancellationToken.None;
		_httpClientFactoryMock.Setup(x => x.CreateClient(Constants.HttpClientName))
